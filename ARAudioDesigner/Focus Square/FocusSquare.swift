...
class FocusSquare: SCNNode {
    ...
    /// Color of the diffuse and emmisions
    static let effectColor = #colorLiteral(red: 1, green: 0.8, blue: 0, alpha: 1)
    ...
    override init() {
        ...
        // Create the focus square segments
        let segments = createFocusSquareSegments()

        // Set the position node and add the segments
    ...
    /*
     Creates the sements and sets their length.
     The focus square consists of eight.
     ...
     s5 |     | s6
         ...
         s7  s8
    */
    private func createFocusSquareSegments() -> [Segment] {
    ...
    /**
     Called when a plane hasn't been found
     */
    private func displayAsClosed(for raycastResult: ARRaycastResult, planeAnchor: ARPlaneAnchor, camera: ARCamera?) {
        performCloseAnimation(flash: anchorsOfVisitedPlanes.contains(planeAnchor) == false)
    ...
    private func performOpenAnimation() {
        guard isOpen == false, isAnimating == false else { return }
